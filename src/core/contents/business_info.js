    ...
    return (
        <HeaderTwo className="container">
            <div className="smMenuPar">
                <button className={`itemsPar ${loc.pathname===`/${params}/businessinfo/1`&&`itemsPar2`}`}><span>Танилцуулга</span></button>
                <button className={`itemsPar ${loc.pathname===`/${params}/businessinfo/2`&&`itemsPar2`}`}><span>Борлуулалтын мэдээлэл</span></button>
                <button className={`itemsPar ${loc.pathname===`/${params}/businessinfo/3`&&`itemsPar2`}`} ><span>Санхүүгийн нөөц, боломж</span></button>
            </div>
