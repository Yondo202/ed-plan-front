...
const ExportThree2 = ({setProductName, modal}) => {
    ...
    const fetchData = () =>{
        axios.get(`exportthrees?parent=${modal?ctx.targetProduct?.id:slug}&idd=${param}`, ).then(res=>{
            if(res.data.length){
    ...
    const clickHandle = () =>{
        if(data.length){
            ctx.loadFunc(true);
            if(fetchID){
                axios.put(`exportthrees/${fetchID}`, { body: data, idd: param, parent: slug, export_product: slug }).then(res=>{
                    ctx.alertFunc('green','Амжилттай',true );
                    ctx.loadFunc(false);
                    history.push(`/${param}/analysis/1/${slug}`);
                }).catch(err=>ctx.alertFunc('orange','Алдаа гарлаа',true ));
            }else{
                axios.post(`exportthrees`, { body: data, idd: param, parent: slug, export_product: slug }).then(res=>{
    ...
    return (
        <>
        {modal? <ContentParser data={data} titleSm={`Өртгийн задаргаа`} />
        :<Container>
            ...
            <CkEditor data={data} setData={setData} title={`Өртгийн задаргаа`} />
            <ButtonStyle2 >
